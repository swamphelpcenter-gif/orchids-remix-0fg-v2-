...
export default function UserProfilePage() {
  ...
  const [newBadge, setNewBadge] = useState<BadgeConfig | null>(null);
  ...
  const markerRef = useRef<any>(null);
    ...
    const [ipInfo, setIpInfo] = useState<{
      ...
      os?: { name: string; version: string; logo: string };
    } | null>(null);
    ...
    const detectBrowser = () => {
        ...
        const browsers = [
            { name: 'Brave', test: () => (navigator as any).brave && (navigator as any).brave.isBrave !== undefined, versionRegex: /Chrome\/([0-9.]+)/ },
        ...
        const detectOS = () => {
            const osMatchers = [
                { name: 'Windows', regex: /Windows NT ([0-9.]+)/, logo: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRawnYVTDpRkKMnWg5nUFY7qT9VxMXuoX9b5Q&s' },
                { name: 'MacOS', regex: /Mac OS X ([0-9._]+)/, logo: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTn8hcR0r6Q1We1raiIHJ1Y95w0FEKYjrdTr4fstZtgJnWrF8gAQrepf_4&s=10' },
                { name: 'Linux', regex: /Linux/, logo: 'https://cdn-icons-png.flaticon.com/512/518/518713.png' },
                { name: 'iOS', regex: /iPhone|iPad/, logo: 'https://cdn-icons-png.freepik.com/512/0/747.png' },
                { name: 'Android', regex: /Android ([0-9.]+)/, logo: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT6LqdG9A0yVP2PEV_oUpQTTuJGTj6wvNSU_DNHdV5RX4RWq5b1o6-CNITo&s=10' },
            ];
    
            for (const os of osMatchers) {
                const match = navigator.userAgent.match(os.regex);
                if (match) {
                    return { name: os.name, version: match[1] || 'Unknown Version', logo: os.logo };
                }
            }
            return { name: 'Unknown OS', version: 'Unknown Version', logo: '' };
        };
        ...
        fetch("https://ipapi.co/json/")
          ...
          .then(data => {
            setIpInfo({
              ...
              browser: { name: browserInfo.name, version: browserInfo.version },
              os: browserInfo.os
            });
          })
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: badgeData } = await supabaseClient.from("badges").select("*");
      const configs: Record<string, BadgeConfig> = {};
      badgeData?.forEach((b: any) => {
        configs[b.name] = { name: b.name, image_url: b.image_url };
      });
      ...
      if (user && user.id === profileData.id) {
        const seenBadgesKey = `seen_badges_${profileData.id}`;
        const seenBadges = JSON.parse(localStorage.getItem(seenBadgesKey) || "[]");
        const currentBadges = [];
        if (profileData.is_verification) currentBadges.push("Verified Badge");
        if (profileData.is_admin) currentBadges.push("Administrator Badge");
        if (profileData.is_content_creator) currentBadges.push("Content Creator Badge");

        const latestBadge = currentBadges[currentBadges.length - 1];
        if (latestBadge && !seenBadges.includes(latestBadge)) {
          setNewBadge(configs[latestBadge]);
          localStorage.setItem(seenBadgesKey, JSON.stringify(currentBadges));
        }
      }
  ...
  return (
    <>
      ...
      <Script src="https://unpkg.com/maplibre-gl@3.6.0/dist/maplibre-gl.js" strategy="afterInteractive" />
                            ...
                            <div className="profile-info-text">
                              <h4 className="mb-1 d-flex align-items-center gap-2 flex-wrap">
                                ...
                                <div className="d-flex align-items-center gap-1">
                                  {profile.is_verification && badgeConfigs["Verified Badge"] && (
                                    <img
                                      src={badgeConfigs["Verified Badge"].image_url}
                                      alt="Verified"
                                      width="20"
                                      height="20"
                                  ...
                                  {profile.is_admin && badgeConfigs["Administrator Badge"] && (
                                    <img
                                      src={badgeConfigs["Administrator Badge"].image_url}
                                      alt="Admin"
                                      width="20"
                                      height="20"
                                      style={{ objectFit: "contain" }}
                                  ...
                                  {profile.is_content_creator && badgeConfigs["Content Creator Badge"] && (
                                    <img
                                      src={badgeConfigs["Content Creator Badge"].image_url}
                                      alt="Content Creator"
                                      width="20"
                                      height="20"
                                      style={{ objectFit: "contain" }}
                ...
                {isOwner && (
                  ...
                  <div className="row mt-6">
                        ...
                        <div className="col-12">
                            <div className="card">
                              <div className="card-header d-flex align-items-center justify-content-between">
                                <h5 className="card-title m-0">Informasi Pribadi</h5>
                                <span className="badge bg-label-danger rounded-pill">Privat</span>
                              </div>
                              <div className="card-body">
                                <div className="row g-4">
                                  <div className="col-md-6">
                                    <div className="d-flex align-items-center mb-4">
                                      <div className="avatar me-3">
                                        <div className="avatar-initial bg-label-primary rounded shadow-xs">
                                          <i className="ri-router-line ri-24px"></i>
                                        </div>
                                      </div>
                                      <div>
                                        <p className="mb-0 text-muted small">IP Pengguna</p>
                                        <div className="d-flex align-items-center gap-2">
                                          <h6 className="mb-0">{showIp ? ipInfo?.ip : maskIp(ipInfo?.ip || "")}</h6>
                                          <button 
                                            className="btn btn-sm btn-text-secondary rounded-pill btn-icon"
                                            onClick={() => setShowIp(!showIp)}
                                          >
                                            <i className={`ri-eye-${showIp ? 'off-' : ''}line`}></i>
                                          </button>
                                        </div>
                                      </div>
                                    </div>
                                    <div className="d-flex align-items-center mb-4">
                                      <div className="avatar me-3">
                                        <div className="avatar-initial bg-label-info rounded shadow-xs">
                                          <i className="ri-broadcast-line ri-24px"></i>
                                        </div>
                                      </div>
                                      <div>
                                        <p className="mb-0 text-muted small">Provider Jaringan</p>
                                        <h6 className="mb-0">{ipInfo?.org || "Memuat..."}</h6>
                                      </div>
                                    </div>
                                    <div className="d-flex align-items-center mb-4">
                                      <div className="avatar me-3">
                                        <div className="avatar-initial bg-label-success rounded shadow-xs">
                                          <i className="ri-map-pin-2-line ri-24px"></i>
                                        </div>
                                      </div>
                                      <div>
                                        <p className="mb-0 text-muted small">Wilayah / Pulau</p>
                                        <h6 className="mb-0">{ipInfo?.region || "Memuat..."}</h6>
                                      </div>
                                    </div>
                                    <div className="d-flex align-items-center">
                                      <div className="avatar me-3">
                                        <div className="avatar-initial bg-label-warning rounded shadow-xs">
                                          <i className="ri-community-line ri-24px"></i>
                                        </div>
                                      </div>
                                      <div>
                                        <p className="mb-0 text-muted small">Kota / Desa</p>
                                        <h6 className="mb-0">{ipInfo?.city || "Memuat..."}</h6>
                                      </div>
                                    </div>
                                  </div>
                                  <div className="col-md-6">
                                    <div className="d-flex align-items-center mb-4">
                                      <div className="avatar me-3">
                                        <div className="avatar-initial bg-label-secondary rounded shadow-xs">
                                          <i className="ri-time-line ri-24px"></i>
                                        </div>
                                      </div>
                                      <div>
                                        <p className="mb-0 text-muted small">Waktu Lokal</p>
                                        <h6 className="mb-0">{ipInfo?.time || "Memuat..."}</h6>
                                      </div>
                                    </div>
                                    <div className="d-flex align-items-center mb-4">
                                      <div className="avatar me-3">
                                        <div className="avatar-initial bg-label-dark rounded shadow-xs overflow-hidden">
                                          {ipInfo?.os?.logo ? (
                                            <img src={ipInfo.os.logo} alt={ipInfo.os.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                          ) : (
                                            <i className="ri-computer-line ri-24px"></i>
                                          )}
                                        </div>
                                      </div>
                                      <div>
                                        <p className="mb-0 text-muted small">Sistem Operasi</p>
                                        <h6 className="mb-0">{ipInfo?.os ? `${ipInfo.os.name} (${ipInfo.os.version})` : "Memuat..."}</h6>
                                      </div>
                                    </div>
                                    <div className="d-flex align-items-center">
                                      <div className="avatar me-3">
                                        <div className="avatar-initial bg-label-info rounded shadow-xs">
                                          <i className="ri-global-line ri-24px"></i>
                                        </div>
                                      </div>
                                      <div>
                                        <p className="mb-0 text-muted small">Browser Anda</p>
                                        <h6 className="mb-0">{ipInfo?.browser ? `${ipInfo.browser.name} (${ipInfo.browser.version})` : "Memuat..."}</h6>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <p className="text-muted small mt-4 mb-0">
                                  ...
                                  Informasi ini hanya dapat dilihat oleh Anda sendiri.
                                </p>
                              </div>
                            </div>
                        </div>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        if (result.isConfirmed) {
                          ...
                          Swal.fire({
                            title: "Error!",
                            text: "Fungsi hapus akun belum tersedia.",
                            icon: "info",
                            confirmButtonColor: "#4361EE",
                            background: document.documentElement.getAttribute("data-bs-theme") === "dark" ? "#1e1e1e" : "#fff",
                            color: document.documentElement.getAttribute("data-bs-theme") === "dark" ? "#fff" : "#000",
                    ...
                    <div className="mb-6 position-relative d-inline-block">
                      ...
                      >
                        <img 
                          ...
                          width="120" 
                          height="120"
                          style={{ filter: 'drop-shadow(0 0 20px rgba(67, 97, 238, 0.3))' }}
                        />
                      </motion.div>
                      <motion.div
                      ...
                      >
                        <span className="badge rounded-pill bg-primary p-2">
                          <i className="ri-medal-line ri-24px"></i>
                        </span>
                      </motion.div>
                    </div>

                    <h3 className="fw-bold mb-2">Selamat!</h3>
                    <p className="text-muted mb-6">
                      Anda baru saja mendapatkan <br />
                      <strong className="text-primary">{newBadge.name}</strong>
                    </p>
                    
                    <div className="bg-light rounded-4 p-4 mb-6">
                      <div className="d-flex align-items-center justify-content-center gap-2 text-muted small">
                        <i className="ri-time-line"></i>
                        <span>Diberikan pada: {new Date().toLocaleString('id-ID', { dateStyle: 'long', timeStyle: 'short' })}</span>
                      </div>
                    </div>
