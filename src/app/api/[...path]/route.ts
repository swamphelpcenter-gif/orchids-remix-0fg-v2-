...
function replaceCreator(obj: any): any {
  if (Array.isArray(obj)) {
    ...
    for (const key in obj) {
      if (key === "creator" && (obj[key] === "apis.visora.my.id" || obj[key] === "vreden")) {
...
async function handleProxy(
  ...
  const url = `https://apis.visora.my.id/api/${targetPath}${searchParamsString ? `?${searchParamsString}` : ""}`;
  ...
  try {
    ...
    headers.set("Referer", "https://apis.visora.my.id/");
